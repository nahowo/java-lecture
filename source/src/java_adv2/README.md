...
# 섹션 12: HTTP 서버 만들기
- 구조를 정리한 글은 [여기](https://nahowo.tistory.com/32)에 있다. 
- PrintWriter
...
# 섹션 13: 리플렉션
### 리플렉션이 필요한 이유
- 섹션 12에서 만든 커맨드 패턴 서블릿은 몇 가지 단점이 있다. 
  1. 하나의 클래스에는 하나의 기능만 만들 수 있다. 
  2. 새로 만든 클래스를 URL 경로와 항상 매핑해야 한다. 
## 클래스와 메타데이터
- 리플렉션(Reflection): 클래스가 제공하는 다양한 정보를 동적으로 분석하고 사용하는 기능
- 리플렉션을 통해 프로그램 실행 중에 클래스, 메서드, 필드 등에 대한 정보를 얻거나 새 객체를 생성하고 메서드를 호출하며 필드의 값을 읽고 쓸 수 있다. 
- 리플렉션을 통해 얻을 수 있는 정보
  - 클래스 메타데이터: 클래스 이름, 접근 제어자, 부모 클래스, 구현된 인터페이스 등
  - 필드 정보: 필드 이름/타입/접근 제어자, 해당 필드의 값을 읽거나 수정
  - 메서드 정보: 메서드 이름/반환 타입/매개변수 정보를 확인, 실행 중에 동적으로 메서드 호출 가능
  - 생성자 정보: 생성자의 매개변수 타입/개수, 동적으로 객체 생성
- 메타데이터 찾기
  1. 클래스에서 찾기
  2. 인스턴스에서 찾기
  3. 문자(패키지, 클래스명)로 찾기
...
- 클래스 메타데이터가 제공하는 getMethod(메서드 이름, 사용하는 매개변수 타입)을 호출하면 원하는 메서드를 찾을 수 있다. 
- 찾아온 Method에 invoke(실행할 인스턴스, 인자)를 호출하면 해당 인스턴스의 메서드를 호출할 수 있다. 
- getMethod에서 메서드 이름은 변수로 전달할 수 있다. 즉 언제든지 동적으로 변경할 수 있기 때문에 해당 방식을 동적 메서드 호출이라고 한다. 
### 필드 변경
- 리플렉션을 사용하면 필드에 직접 접근해 변경할 수 있다. private 필드더라도 접근 권한을 허용해 변경할 수 있다. 
### 주의점
- 리플렉션을 사용하면 접근 권한을 변경할 수 있으므로 객체 지향 프로그래밍 원칙을 위반할 수 있다. 따라서 리플렉션을 사용할 때는 가능한 경우에만 사용하는 것이 바람직하다.
- 또한 리플렉션을 사용해 변경을 수행하면 컴파일 오류로 잡지 못하는 문제가 발생한다. 

# 섹션 14: 어노테이션
- 남은 문제점
  - 요청 이름과 메서드를 다르게 할 수 없음
  - 메서드 이름으로 처리하기 어려운 URL도 있음
### 어노테이션
- 어노테이션(Annotation)은 프로그램 실행 중에 읽어서 사용할 수 있는 주석이다. Annotation은 주석, 메모를 의미한다. 
- 어노테이션은 `@interface` 키워드로 정의한다. 
- **데이터 타입**
  - 기본 타입, String, `Class`(메타데이터) 또는 인터페이스, ENUM, 다른 어노테이션 타입, 위 타입들의 배열만 가질 수 있다. 
  - 일반적인 클래스는 사용할 수 없다. 
- **default 값**
  - 각 데이터 타입에 default를 지정할 수 있다. 
    `String value() default "";` 형식으로 적용한다. 
- **요소 이름**
  - 메서드 형태로 정의된다. 매개변수는 지정할 수 없다. 
- **반환값**
  - void를 반환 타입으로 사용할 수 없다. 
- **예외**
  - 예외를 선언할 수 없다. 
- **value 요소**
  - value라는 이름의 요소를 하나만 가질 경우 어노테이션 사용 시 `@{어노테이션}(value = {값})` 대신 `@{어노테이션}({값})`으로 생략이 가능하다. 
### 메타 어노테이션
- 메타 어노테이션이란 노테이션을 정의하는 데 사용되는 어노테이션이다.
- `@Retention`
  - 어노테이션의 생존 기간을 지정한다. 
  - value로 RetentionPolicy(ENUM)을 가진다. 
  - `RetentionPolicy.CLASS`: 기본값, 컴파일된 .class 파일에는 남아있지만 자바 실행 시점에 제거된다. 
  - `RetentionPolicy.SOURCE`: 어노테이션이 소스 코드에만 남아있고 컴파일 시 사라진다. 
  - `RetentionPolicy.RUNTIME`: 자바 실행 중에도 남아있다.
- `@Target`
  - 어노테이션을 붙이는 위치를 지정한다. TYPE, METHOD, FIELD등이 있다.
- `@Documented`
  - 자바 API 문서를 만들 때 해당 어노테이션이 함께 포함되는지 지정한다. 
- `@Inherited`
  - 자식 클래스가 어노테이션을 상속받을 수 있도록 한다. 
### 어노테이션과 상속
- 모든 어노테이션은 `java.lang.annotation.Annotation` 인터페이스를 묵시적으로 상속받는다. 
- 어노테이션은 다른 어노테이션이나 인터페이스를 직접 상속할 수 없다. 위의 인터페이스만 상속한다. 
- `@Inherited`
  - 어노테이션 정의 시 `@Inherited` 메타 어노테이션을 붙이면 어노테이션을 적용한 클래스의 자식도 해당 어노테이션을 부여받을 수 있다. 
  - 클래스 상속에서만 적용되고 인터페이스 구현체에는 적용되지 않는다. 
- **`@Inherited`가 클래스 상속에서만 적용되고 인터페이스 구현체에는 적용되지 않는 이유**
1. 클래스 상속과 인터페이스 구현의 차이
   - 클래스 상속은 자식이 부모의 속성/메서드를 상속받는 개념이다. 즉 자식이 부모의 특성을 이어받는 의미이므로 어노테이션 상속도 논리적으로 맞다. 
   - 인터페이스는 메서드의 시그니처를 정의할 뿐 상태나 행위를 가지지 않기 때문에 구현체가 어노테이션을 상속한다는 개념이 맞지 않는다. 
2. 인터페이스와 다중 구현, 다이아몬드 문제
   - 인터페이스는 다중 구현이 가능하고, 어노테이션이 상속 가능하다면 어노테이션 간 충돌이나 모호한 상황이 발생할 수 있다. 
### 어노테이션을 통한 검증
- 검증용 어노테이션과 검증기를 추가하면 어떤 객체든지 간단하게 검증할 수 있어 가독성/확장성이 향상된다. 
- 자바 진영에서느 어노테이션 기반 검증 기능을 Jakarta Bean Validation이라는 이름으로 표준화했다.
## 자바 기본 어노테이션
### `@Override`
- 메서드 재정의가 정확하게 되었는지 컴파일러가 체크하는 데 사용한다. 오버라이드하려는 메서드가 잘못 정의되어 있으면 컴파일 오류를 발생시킨다. 
- RetentionPolicy.SOURCE를 사용해 컴파일 단계에서는 사라진다. 
### `@Deprecated`
- 더 이상 사용되지 않는 기능을 의미한다. 해당 어노테이션이 적용된 기능은 사용을 지양하자. 
### `@SuppressWarnings`
- 경로를 무시하는 어노테이션이다. unused, deprecated, rawtypes, unchecked, serial 등의 경고를 무시하도록 할 수 있다. 

# 섹션 15: HTTP 서버 활용
- 3가지 오류
  - 컴파일 오류: 가장 좋은 오류, 프로그램 실행 전에 개발자가 문제 확인 가능
  - 런타임 오류 - 시작 오류: 자바 프로그램/서버 시작 시점에 발견하는 오류, 문제를 빠르게 발션할 수 있으므로 좋은 오류, 고객이 문제를 인지하기 전에 수정/해결 가능
  - 런타임 오류 - 작동 오류: 고객이 특정 기능을 사용할 때 발생하는 오류, 원인 파악/문제 해결에 가장 많은 시간이 걸림