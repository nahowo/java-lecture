...
- HTTP에 HTML, text, 이미지, 음성, 영상, 파일, JSON등 거의 모든 형태의 데이터 전송 가능
### HTTP 요청 메시지
- 시작 라인
  - HTTP 메서드: 요청 방식
  - 요청 대상: url(절대 경로), 쿼리 
  - HTTP 버전
- 헤더
  - HTTP 전송에 필요한 모든 부가 정보: 바디 내용, 크기, 압축, 인증 정보, 요청 클라이언트 정보, 서버 애플리케이션 정보, 캐시 관리 정보 등
- 공백 라인
  - 헤더와 바디 사이 공백 1줄
- 바디
...
- 메시지 바디는 사용하지 않음(데이터 전달은 가능하지만 지원하지 않는 곳이 많아서 권장하지 않음)
## POST
- 요청 데이터 처리
