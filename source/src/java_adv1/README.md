...
- 서비스에 문제가 없도록 안정적으로 종료하는 것을 우아한 종료라고 한다. 
- ExecutorService의 종료 메서드
  - shutdown(): 새 작업을 받지 않고 이미 제출된 작업을 모두 완료한 뒤 종료, 논블로킹 메서드
  - shutdownNow(): 실행중인 작업을 중단하고 대기중인 작업을 반환하며 즉시 종료, 실행중인 작업을 중단하기 위해 인터럽트 발생, 논블로킹 메서드
  - close(): 자바 19부터 제공하는 shutdown(), shutdown() 호출 뒤 하루를 기다려도 작업이 완료되지 않으면 shutdownNow() 호출
- 기본적으로 우아한 종료를 선택하고, 우아한 종료가 되지 않으면 강제 종료를 수행하자. 

## Executors의 풀 전략
### 고정 풀 전략
- `newFixedThreadPool`을 사용한다.  코어 스레드 수가 고정되고 대기 큐는 무한대로 증가한다. 
- 초과 스레드는 대기 큐에 쌓여 CPU, 메모리 리소스를 어느정도 예측할 수 있다. 
- 요청이 처리되는 시간보다 쌓이는 시간이 빨라지면 서버 자원을 효율적으로 이용하지 못한다는 문제가 생긴다. 
### 캐시 풀 전략
- `newCachedThreadPool`을 사용한다. 모든 요청이 대기하지 않고 스레드가 바로 처리한다. 큐(SynchronousQueue)에 작업을 저장하지 않는다.
- SynchronousQueue: 생산자 작업을 소비자 스레드에 직접 전달한다. 생산자와 소비자를 동기화하여 생산자는 소비자가 작업을 요청할 때까지, 소비자는 생산자에게 작업을 받아올 때까지 블로킹 상태를 유지한다. 
- **코어 스레드가 없다. 바로 초과 스레드가 생성된다.** 초과 스레드는 생존 기간동안 생존하고 그 기간동안 새로 요청되지 않으면 제거된다. 
- CPU 메모리 자원만 허용한다면 시스템 자원을 최대로 사용할 수 있다. 하지만 서버가 감당할 수 있는 임계점을 넘는 순간 시스템이 다운될 수 있다. 
### 사용자 정의 풀 전략
- 요청 상황을 세분화하여 상황별 대응법을 세운다. 
  - 일반: 일반적인 상황에는 고정 풀 전략을 사용해 서비스 안정성을 높인다. 
  - 긴급: 사용자 요청이 급증하면 스레드를 추가로 투입하여 작업을 빠르게 처리한다. 
  - 거절: 사용자 요청이 폭증하여 긴급 대응도 어렵다면 요청을 거절한다. 

## Executor 예외 정책
- AbortPolicy: 초과 시 예외 발생
- DiscardPolicy: 초과 시 요청을 버림
- CallerRunsPolicy: 요청 스레드에서 해당 요청을 처리