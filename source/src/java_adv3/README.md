...
# 섹션 10: 스트림 API 3 - 컬렉터
- 스트림의 결과물이 필요한 경우 컬렉터Collectors를 활용한다. 
## 다운스트림 컬렉터
- 그룹화된 이후 각 그룹 내부에서 추가적인 연산, 결과물을 정의하는 역할이 다운스트림 컬렉터이다.
...
# 섹션 12: 디폴트 메서드
- 인터페이스에 새로운 메서드를 추가하면 이미 배포된 기존 구현 클래스들에 컴파일 오류가 발생하는 문제가 있었다.
- 이를 해결하기 위해 자바 8에서 디폴트 메서드가 등장했다.
### 디폴트 메서드
- 자바가 처음 등장했을 때 인터페이스는 구현 없이 메서드 시그니처만 정의하는 용도로 사용되었다.
- 도입 이유
  - 하위 호환성: 인터페이스에 새 메서드 추가 시 기존 코드 수정 없음
  - 라이브러리 확장성: 표준 라이브러리에 변경이 있어도 사용자/서드파티 라이브러리 구현체의 수정 필요 x
  - 람다, 스트림 API 연계
  - 설계 유연성 향상
- `default` 키워드를 사용해 정의한다. 
- 사용법
  - 하위 호환성을 위해 최소한으로 사용한다. 
  - 인터페이스는 여전히 추상화 역할을 하도록 유지한다. 
  - 다중 상속(충돌) 문제를 해결한다. 즉 서로 다른 인터페이스에 동일한 시그니처의 디폴트 메서드가 존재하면 오버라이드를 통해 직접 처리해야 한다(직접 구현하거나 super를 사용해 어떤 인터페이스를 사용할지 명시). 
  - 디폴트 메서드에 상태를 두지 않는다.

# 섹션 13: 병렬 스트림
### fork/join 패턴
- 하나의 큰 작업을 여러 스레드가 처리할 수 있는 작은 단위의 작업으로 분할해 처리하도록 하는 **분할(fork) -> 처리(execute) -> 모음(join) 단계의 멀티스레딩 패턴**을 말한다. 
- 자바는 fork/join 프레임워크를 제공해 개발자가 이러한 패턴을 더 쉽게 구현할 수 있도록 지원한다. 
### fork/join 프레임워크
...
- **작업 훔치기(work stealing) 알고리즘**
  - 바쁜 스레드의 작업을 작업이 없는 스레드가 훔쳐와 대신 처리 -> 부하 균형 자동 조절
### fork/join 공용 풀(Common Pool)
- 자원 관리: 시스템에서 자동으로 관리
- 재사용성: 여러 곳에서 공유 관리
- 설정 제어: 기본 설정 사용, 설정 변경은 지양
- 라이프사이클: 명시적으로 종료하는 커스텀 풀과 달리 JVM이 관리, 종료 필요 없음
## 자바 병렬 스트림
- 스트림에서 parallel()을 선언하면 스트림은 공용 ForkJoinPool을 사용한다. 즉 스트림을 병렬로 처리하는 것이 가능하다. 
### 병렬 스트림 사용 시 주의점
- Fork/Join 프레임워크는 CPU 바운드 작업(계산 집약적)에만 사용하기
  - I/O 작업처럼 블로킹 대기 시간이 긴 작업을 병렬 처리하면 아래와 같은 문제가 발생한다. 
...
# 섹션 14: 함수형 프로그래밍
- 프로그램을 함수를 조합해 만드는 방식에 초점을 두는 프로그래밍 패러다임
### 함수형 프로그래밍의 핵심 개념/특징
- 선언형 접근: 필요한 결과를 얻기 위해 무엇(what)을 계산할 것인지를 강조
  - 복잡한 제어 구조, 상태 관리를 함수의 합성과 호출로 대체 -> 간결, 가독성 높은 코드
- 함수 합성(Composition)
  - 간단한 함수를 조합해 더 복잡한 함수
- 지연 평가(선택적 특징)
  - 불필요한 계산 비용 감소
- 함수를 일급 시민(First-class Citizen)으로 취급
  - 함수가 일반 값과 동일한 지위
  - 함수를 일반 값처럼 변수에 대입, 인자로 전달, 함수에서 반환 등 고차 함수 사용이 자유로움
- 불변 상태 지향
  - 생성 후 가능한 변경 X, 변경이 필요한 경우 새 값을 생성해 사용
- 순수 함수(Pure Function)를 중심
  - 같은 인자를 주면 항상 같은 결과를 반환하는 함수
  - 외부 상태에 의존하거나 변경하는 부수 효과(side effect) 가 없는 함수
- 부수 효과 최소화
  - 변수, 객체 변경을 자양 -> 프로그램 버그 감소, 테스트/병렬 처리/동시성 지원 용이
### 장단점
- 장점
  - 코드 가속성, 유지보수성
  - 병렬 처리 및 동시성
