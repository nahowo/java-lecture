...
## 상속과 메모리 구조
- A를 상속하는 B의 인스턴스를 생성하면, B의 인스턴스를 생성하는 동시에 B의 부모인 A의 인스턴스까지 포함해 생성한다. 두 인스턴스는 다른 객체로 저장된다. 
...
- 접근 제어자는 상위 클래스의 메서드보다 덜 제한적이어야 한다. 
  ...
  - 하위 클래스 인스턴스 생성 시 상위 클래스의 인스턴스도 함께 생성하는데, 두 인스턴스가 별개로 저장되기 때문에 접근 제어자가 적용된다. 
...
# 섹션 11: 다형성1
- 다형성(Polymorphism)은 다양한 형태, 여러 형태를 의미한다. 다형성을 사용하면 하나의 객체가 다른 타입으로 사용될 수 있다. 

## 다형적 참조
- 부모 타입의 변수가 자식 인스턴스를 참조하는 것을 의미한다. 
  ```java
  Parent poly = new Child();
  ```
  - 부모 타입의 변수가 자식 인스턴스를 참조한다. 
  - new 키워드로 Child 인스턴스를 만들었다.
  - 자식 타입을 생성했기 때문에 메모리에 Parent, Child 모두 생성된다. 
- **부모는 자식을 담을 수 있다.** 

### 다형적 참조와 인스턴스 실행
- 부모 타입은 자식 타입의 메서드를 사용할 수 없다. 
  ```java
  Parent poly = new Child();
  ```
  - 이때 메모리에는 Parent, Child 모두 생성된다. 하지만 poly의 타입이 Parent이기 때문에 메서드 수행 시 메모리의 Parent에 가장 먼저 접근한다. 메서드를 찾지 못하면 Parent의 부모 방향으로만(상위 클래스로만) 이동하기 때문에 Child가 메모리에 있다고 하더라도 하위 타입인 이상 메서드를 사용할 수는 없다.

## 다형성과 캐스팅
### 캐스팅
- 다운캐스팅
  - 자식 타입으로 변경
  - 호출하는 타입을 하위 타입으로 변경하면 하위 클래스의 메서드를 사용할 수 있다. 
- 업캐스팅
  - 부모 타입으로 변경
  - (Parent)는 생략할 수 있다. 업캐스팅은 자주 사용하기 때문에 생략하는 것이 편하다. 
- 일시적 다운캐스팅
  - 담아둘 변수를 생성하지 않고 괄호를 사용해 메서드를 사용하는 순간만 다운캐스팅을 수행할 수 있다(참조값 저장 없음).
...
```
- 위 코드를 실행하면 ClassCastException이라는 런타임 오류가 발생한다.
- parent2를 생성하면 메모리에는 Parent 인스턴스만 생성되고, Child 타입으로 다운캐스팅 시 Child가 메모리 상에 존재하지 않기 때문에 문제가 발생한다. 
- 자바에서는 사용할 수 없는 타입으로 다운캐스팅하는 경우 ClassCastException 예외를 발생시킨다. 
- 업캐스팅은 상위 타입의 인스턴스를 모두 메모리에 생성하기 때문에 문제가 생기지 않는다. 

### instanceof
- 다형성에서 참조형 변수는 다양한 자식을 대상으로 참조할 수 있다. 이 때 어떤 인스턴스를 참조하고 있는지 확인하기 위해 `instanceof` 키워드를 사용한다.  
- A instanceof B는 B 타입에 A 인스턴스 타입이 들어갈 수 있는지 여부를 의미한다. 
...
- **오버라이딩된 메서드가 항상 우선권을 가진다.**  
  ...
  ```
  - poly는 Parent 타입이지만 Child에서 Parent의 method()를 오버라이딩했다면 오버라이딩한 메서드를 실행한다. 
    1. poly의 타입인 Parent에서 method()를 찾는다. 
    2. Parent의 하위 타입인 Child에 method()가 오버라이딩되어 있고, 이 오버라이딩된 메서드가 우선권을 가진다. 
    3. 따라서 Child의 method()가 실행된다. 

# 섹션 12: 다형성2
- 타입이 다른 경우 같은 동작을 중복 없이 수행하기 위해 다형성을 사용한다. 
- 메서드로 추출하는 단축키: option + cmd + m

## 추상 클래스(Abstract Class)
- 부모 클래스는 제공하지만 실제 생성되지는 않는 클래스이다. 
- 인스턴스가 존재하지 않고 상속 목적으로만 사용된다. 부모 클래스 역할을 한다. 
- `abstract` 키워드를 클래스 앞에 붙여 사용한다. 

### 추상 메서드
- 추상적인 개념을 제공하는 메서드이다. 실체(메서드 바디)가 존재하지 않는다. 
- `abstract` 키워드를 메서드 앞에 붙여 사용한다.
- **추상 메서드가 하나라도 있는 클래스는 추상 클래스로 선언해야 한다.** 
- **추상 메서드는 상속받는 자식 클래스에서 반드시 오버라이딩해야 한다.** 오버라이딩하지 않는다면 자식도 추상 클래스여야 한다. 
- 추상 클래스는 위의 제약을 제외하고는 일반적인 클래스와 동일하다. 

### 순수 추상 클래스
- 모든 메서드가 추상 메서드인 추상 클래스이다. 구현된 로직 없이 다형성을 위한 껍데기 역할을 제공한다. 
- 자식은 모든 메서드를 오버라이딩해야 한다. 
- 부모의 기능을 물려받는 게 아니라 규격에 맞추는 개념이다. 자바는 이런 규격에 맞추도록 하는 순수 추상 클래스를 더 편리하게 사용할 수 있도록 인터페이스를 제공한다. 

## 인터페이스
- `interface` 키워드를 사용해 정의한다.
- 인터페이스는 순수 추상 클래스의 개념과 동일하다. 
- 모든 메서드는 `public abstract`를 가진다(생략을 권장한다). 
- 다중 구현(다중 상속)을 지원한다. 
- 멤버 변수를 사용할 수 있다. 이때 멤버 변수는 `public static final`이 들어간다. 즉 인스턴스와 관계가 없는 상수이다.
- 모든 메서드가 추상 메서드이므로 상속보다는 자식이 구현하는 개념에 더 가깝다. 그래서 상속(extends) 대신 구현(implements) 단어를 사용한다. 
- 인터페이스를 사용해야 하는 이유
  1. **제약**: 인터페이스를 구현하는 곳에서 인터페이스의 메서드를 반드시 구현하라는 규약 전달 -> 추상 클래스 사용 시 추상 메서드가 아닌 메서드를 추가할 경우 순수 추상 클래스의 개념이 깨지게 됨
  2. **다중 상속**: 부모를 여러 명 두는 다중 구현(다중 상속)이 가능하다. 
- **좋은 프로그램은 적절한 제약이 있는 프로그램이다.** 

## 인터페이스 - 다중 구현
- 자바는 다중 상속을 지원하지 않는다. 만약 A 클래스가 B, C 클래스를 상속하고, B, C에 동일한 메서드가 존재한다면 A에서 해당 메서드를 호출했을 때 어떤 클래스의 메서드를 호출할지 애매해진다(다이아몬드 문제). 또한 다중 상속을 허용하면 클래스 계층 구조가 매우 복잡해질 수 있다. 
- 인터페이스는 메서드가 전부 추상 메서드이기 때문에 다중 상속을 허용한다. 어차피 자식 클래스에서 구현해야 하기 때문에, 다이아몬드 문제가 발생하지 않는다. 즉 B, C에 같은 이름이 있어도 어차피 A에서 구현한 메서드를 호출하기 때문에 B, C 메서드에 접근하지 않는다.

## 클래스와 인터페이스 활용
- 클래스를 상속하면서 인터페이스들을 구현할 수 있다. 

# 섹션 13: 다형성과 설계
- 역할과 구현으로 구분하면 단순하고 유연하며 변경이 편리한 구조를 만들 수 있다. 
- 다형성을 통해 인터페이스를 구현한 객체 인스턴스를 실행 시점에 유연하게 변경(선택)할 수 있다.
- 역할(인터페이스)가 바뀌면 큰 변경이 발생하기 때문에 인터페이스를 안정적으로 설계하는 것이 중요하다.
## OCP(Open-Close Principle)
- 확장에는 열려 있되 코드 수정에는 닫혀 있어야 한다는 원칙이다. 